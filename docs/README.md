...
## Audited V2.1 corrections
- PDF.js is loaded once and coordinate extraction no longer performs repeated dynamic imports.
- OCR word coordinates are retained, allowing suggested boxes on image-only PDF pages.
- Same-line and below-label IPA values are handled separately.
- Manual boxes remain selected after drawing.
- CSV cells beginning with formula characters are neutralised for safer Excel opening.
